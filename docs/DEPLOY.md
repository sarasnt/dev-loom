...
can't collide with them. Certificates generate themselves on first `up` (the `edge-certs` one-shot service); trust
`edge/certs/devloom-local-ca.crt` afterwards to use the `.dev` names.
